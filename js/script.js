$(document).ready(function(){
    ...
    var elm = ".box"; //각 섹션에 지목할 클래스명을 저장
    //$(elm).css("background", "#08f");
    $(elm).each(function(index){ //0 1 2 3 4 5 6
        //개별 섹션에서 접근하여 이벤트를 발생시킴
        $(this).on("mousewheel DOMMouseScroll", function(e){
            ...
            if(event.wheelDelta){
                ...
                //크롬 브라우저 기준으로 내렸을 때 delta = -120(또는 -3) / 120 = -1(또는 -3/120) => 음수
                //크롬 브라우저 기준으로 휠 올렸을 때 delta = 120(또는 3) / 120 = 1(또는 3/120) => 양수
                if(window.opera){ //오페라 브라우저는 크롬브라우저와 반대값으로 전달
            ...
            //delta값이 음의 실수 일때는 => 내린다. => 다음 섹션(현재 보여지는 화면보다 아래)이 나와야함
            //delta값이 양의 실수 일때는 => 올린다. => 이전 섹션(현재 보여지는 화면보다 위)이 나와야 함

            var moveTo = $(window).scrollTop(); //각 섹션별로 상당으로부터 절대위치를 파악하기 위함
            ...
            if(delta < 0){
                ...
                try{
                    ...
                    console.log(e);  //타입에러 파트를 에러로 검사창에 노출시키는 것이 아닌 강제로 숨김처리 => 시스템의 에러도 에러로 발생기키는 것을 막는 기능
                }
                ...
                //마우스 휠을 올리는 시점에서 이전 섹션이 존재한다면
                    if($(elmIndex).prev() != undefined){
                        moveTo = $(elmIndex).prev().offset().top; //이전섹션의 상단 수직방향 절대 위치값
    ...
    //키보드 이벤트 통해서 fullpage 슬라이드 제어
    //$(선택자).이벤트명(fun(){실행문}); ==> 기존에 어떤 완성이 종료된 시점에서 접근시 작성 ==> 이벤트관련 실행에 대한 메모리 값(실행문)을 브라우저가 로딩시 저장
    //$(document).on("이벤트명", "선택자", fun(){실행문}); ==> 기존에 완성된 구조가 존재하는 말든 상관없이 이벤트가 발생되는 시점까지 무조건 기다린다.(DOM이라는 구조에서) ==> 이벤트관현 실행에 대한 메모리 값(실행문)을 이벤트가 발생되는 시점에서 저장
    ...
    $(document).on("keydown", function(evt){
        ...
        if(key_num == 40 || key_num == 34){ //하방향키 $pageDown 키보드 눌렀을 때
            try{
            ...
            try{
                $("html, body").stop().animate({scrollTop : $(elm).first().offset().top}, 500, function(){
                    $(elm).removeClass("active");
            ...
            try{
                $("html, body").stop().animate({scrollTop : $(elm).last().offset().top}, 500, function(){
                    $(elm).removeClass("active");
